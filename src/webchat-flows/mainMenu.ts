...
import { FlowButton, Triggers, WebchatFlow } from 'automation-sdk';


export function webchatMainMenu() {
    const fixedQuickReplies = new WebchatFlow("partner_mainMenu", "greeting");
    fixedQuickReplies
        ...
        .quickReply("{{translate.GREETING_FLOW_MESSAGE}}", [
            new FlowButton("4", "{{translate.TICKETING}}", "ticket_creation",
                new WebchatFlow("ticket_creation", "sub").fire(Triggers.INTENT, "CREATE_TICKET")),
            new FlowButton("5", "{{translate.TALK_TO_AGENT_BUTTON}}", "talk_to_agent",
                new WebchatFlow("talk_to_agent", "sub").fire(Triggers.INTENT, "HELP")),
            new FlowButton("6", "{{translate.END_CONVERSATION_BUTTON}}", "end_conversation",
                new WebchatFlow("end_conversation", "sub").text([['{{translate.END_CONVERSATION}}']])),
        ])
